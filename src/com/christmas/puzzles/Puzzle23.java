...
public class Puzzle23 {
    ...
    public static int solve() {
        ...
        traverseAllPaths(adjList, new HashSet<>(), "start");

        return pathCount;
    }

    private static void traverseAllPaths(Map<String, List<String>> adjList, Set<String> visited, String node) {
        if(node.equals("end"))
            pathCount++;
        ...
        for(String adj: adjList.get(node)) {
            if(!visited.contains(adj)) {
                traverseAllPaths(adjList, new HashSet<>(visited), adj);
            }
        }
    }
}
